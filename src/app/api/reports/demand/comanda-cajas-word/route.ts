...
import { generateComandaCajasWord } from '@/lib/word/generators-demand';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const buffer = await generateComandaCajasWord(demandRows, { faltantes });
    const suffix = faltantes ? '-faltantes' : '';

    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
        'Content-Disposition': `attachment; filename="comanda-cajas${suffix}.docx"`,
        'Cache-Control': 'no-store',
      },
    });
  } catch (error) {
    console.error('Error generating demand Comanda Cajas Word:', error);
