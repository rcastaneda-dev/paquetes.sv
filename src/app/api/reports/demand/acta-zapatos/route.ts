...
import { generateActaRecepcionZapatosPDFFromDemand } from '@/lib/pdf/generators-demand';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pdfStream = generateActaRecepcionZapatosPDFFromDemand(demandRows, { faltantes });
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        'Content-Disposition': `inline; filename="acta-recepcion-zapatos${suffix}.pdf"`,
        'Cache-Control': 'no-store',
      },
    });
  } catch (error) {
    console.error('Error generating demand Acta Zapatos PDF:', error);
