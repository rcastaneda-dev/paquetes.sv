...
import { generateComandaUniformesPDFFromDemand } from '@/lib/pdf/generators-demand';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pdfStream = generateComandaUniformesPDFFromDemand(demandRows, { faltantes });
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        'Content-Disposition': `inline; filename="comanda-uniformes${suffix}.pdf"`,
        'Cache-Control': 'no-store',
      },
    });
  } catch (error) {
    console.error('Error generating demand Comanda Uniformes PDF:', error);
