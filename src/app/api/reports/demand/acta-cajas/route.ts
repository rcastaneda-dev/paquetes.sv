...
import { generateActaRecepcionCajasPDFFromDemand } from '@/lib/pdf/generators-demand';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pdfStream = generateActaRecepcionCajasPDFFromDemand(demandRows, { faltantes });
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        'Content-Disposition': `inline; filename="acta-recepcion-cajas${suffix}.pdf"`,
        'Cache-Control': 'no-store',
      },
    });
  } catch (error) {
    console.error('Error generating demand Acta Cajas PDF:', error);
