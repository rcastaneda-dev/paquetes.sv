...
import { generateCajasDemandExcel } from '@/lib/excel/generators-demand';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const buffer = await generateCajasDemandExcel(demandRows);
    const suffix = faltantes ? '-faltantes' : '';

    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': `attachment; filename="cajas-acumulado${suffix}.xlsx"`,
        'Cache-Control': 'no-store',
      },
    });
  } catch (error) {
    console.error('Error generating demand Cajas Excel:', error);
