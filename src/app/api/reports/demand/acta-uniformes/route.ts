...
import { generateActaRecepcionUniformesPDFFromDemand } from '@/lib/pdf/generators-demand';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pdfStream = generateActaRecepcionUniformesPDFFromDemand(demandRows, { faltantes });
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        'Content-Disposition': `inline; filename="acta-recepcion-uniformes${suffix}.pdf"`,
        'Cache-Control': 'no-store',
      },
    });
  } catch (error) {
    console.error('Error generating demand Acta Uniformes PDF:', error);
