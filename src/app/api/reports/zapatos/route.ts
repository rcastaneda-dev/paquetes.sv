...
import { generateZapatosPDF } from '@/lib/pdf/generator';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pdfStream = generateZapatosPDF({
      fechaInicio,
      students,
    });
    ...
    const fileName = `zapatos-${slugify(school.nombre_ce)}-${slugify(gradoLabel)}.pdf`;
    ...
    console.error('Error generating Zapatos PDF:', error);
