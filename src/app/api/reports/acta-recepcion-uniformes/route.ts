...
import { generateActaRecepcionUniformesPDF } from '@/lib/pdf/generator';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pdfStream = generateActaRecepcionUniformesPDF({
      fechaInicio,
      students,
    });
    ...
    const fileName = `acta-recepcion-uniformes-${slugify(school.nombre_ce)}-${slugify(gradoLabel)}.pdf`;
    ...
    console.error('Error generating Acta Recepción Uniformes PDF:', error);
