...
import { generateCamisasPDF } from '@/lib/pdf/generator';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const pdfStream = generateCamisasPDF({
      fechaInicio,
      students,
    });
    ...
    const fileName = `camisas-${slugify(school.nombre_ce)}-${slugify(gradoLabel)}.pdf`;
    ...
    console.error('Error generating Camisas PDF:', error);
