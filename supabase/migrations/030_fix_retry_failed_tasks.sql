-- ========================================================================
-- Migration: Fix retry_failed_tasks to remove report_zip_parts reference
-- ========================================================================
-- Purpose:
-- - Update retry_failed_tasks function to work without report_zip_parts table
-- - The table was removed in migration 022_remove_zip_parts.sql
-- - Remove zip_parts_deleted from return values
-- ========================================================================

BEGIN;

-- Drop and recreate the retry_failed_tasks function without zip_parts reference
DROP FUNCTION IF EXISTS public.retry_failed_tasks(UUID);
...
BEGIN
  ...
  IF v_tasks_retried = 0 THEN
    RETURN QUERY SELECT false, 'No failed tasks found to retry'::TEXT, 0;
    RETURN;
  END IF;
  ...
  -- Note: Old zip artifacts in storage can be cleaned up manually if needed
...
-- - No longer references report_zip_parts (removed in migration 022)
-- ========================================================================
